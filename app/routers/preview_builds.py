...
def get_build_service():
    ...
    return PreviewBuildService(Path(settings.data_directory).resolve() / 'preview-builds',
                               get_processing_service(), settings.upload_directory)
...
async def invoke(fn, *args):
    ...
    except (CommitmentValidationError, PackageError, PolicyError, OriginError, SigningError, LifecycleError) as exc:
...
@router.post('')
async def create(request: Request, owner=Depends(authenticated_owner), service=Depends(get_build_service)):
...
async def rows(job_id: str, start: int = 0, count: int = 25, owner=Depends(authenticated_owner), service=Depends(get_build_service)):
...
async def cancel(job_id: str, request: Request, owner=Depends(authenticated_owner), service=Depends(get_build_service)):
    await options(request, EmptyOptions)
    return await invoke(service.cancel, job_id, owner)

@router.put('/{job_id}/selection')
async def selection(job_id: str, request: Request, owner=Depends(authenticated_owner), service=Depends(get_build_service)):
    return await invoke(service.select, job_id, owner, await options(request, Selection))

@router.post('/{job_id}/policy')
async def policy(job_id: str, request: Request, owner=Depends(authenticated_owner), service=Depends(get_build_service)):
    result = await invoke(service.scan, job_id, owner, await options(request, Consent))
    return result['policy']

@router.post('/{job_id}/package')
async def package(job_id: str, request: Request, owner=Depends(authenticated_owner), service=Depends(get_build_service)):
...
async def download(job_id: str, owner=Depends(authenticated_owner), service=Depends(get_build_service)):
    payload = await invoke(service.download, job_id, owner)
    return Response(payload, media_type=MEDIA_TYPE, headers={'Cache-Control': 'no-store', 'Content-Disposition': 'attachment; filename="preview.json"'})

@router.post('/{job_id}/origin-check')
async def origin_check(job_id: str, request: Request, owner=Depends(authenticated_owner), service=Depends(get_build_service)):
...
async def candidate(job_id: str, request: Request, owner=Depends(authenticated_owner), service=Depends(get_build_service)):
    return await invoke(service.candidate, job_id, owner, await options(request, CandidateOptions))

@router.post('/{job_id}/submit')
async def submit(job_id: str, request: Request, owner=Depends(authenticated_owner), service=Depends(get_build_service)):
    await options(request, EmptyOptions)
    return await invoke(service.submit, job_id, owner)

@router.post('/{job_id}/withdraw')
async def withdraw(job_id: str, request: Request, owner=Depends(authenticated_owner), service=Depends(get_build_service)):
    await options(request, EmptyOptions)
    return await invoke(service.withdraw, job_id, owner)

@router.post('/{job_id}/refresh')
async def refresh(job_id: str, request: Request, owner=Depends(authenticated_owner), service=Depends(get_build_service)):
    return await invoke(service.refresh, job_id, owner, await options(request, CandidateOptions))
