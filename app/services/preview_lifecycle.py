...
from app.services.preview_signing_service import (
    LocalCandidate, SigningError, request_bytes, request_digest, closed,
)
from app.models.preview_disclosure_schemas import DisclosureBinding
from app.services.dataset_merkle_service import canonical_json_bytes, canonical_rfc3339_utc
...
TRANSITIONS = {
    ...
    'signed_candidate': {'submitted', 'submission_unknown', 'withdrawal_pending', 'invalidated', 'retirement_pending'},
...
def capture_rights(text, code, permission):
    if type(text) is not str or not text.strip() or len(text) > 4096 or code not in {'owner', 'licensed', 'public_domain', 'other_authorized'} or permission is not True:
        raise LifecycleError('rights_required')
    return dict(rights_basis_digest=hashlib.sha256(b'aim-preview-rights-basis-v1\0' + unicodedata.normalize('NFC', text).encode('utf-8')).hexdigest(),
                rights_basis_code=code, public_preview_permission=True)


def stale_at(attested_at, cadence_days):
    stamp = datetime.fromisoformat(canonical_rfc3339_utc(attested_at).replace('Z', '+00:00'))
    if cadence_days is not None and (type(cadence_days) is not int or cadence_days <= 0):
...
def freshness(*, attested_at, cadence_days, now, approval_expires_at=None, policy_expires_at=None, summary_expires_at=None):
    threshold = stale_at(attested_at, cadence_days)
    expiries = [v for v in (approval_expires_at, policy_expires_at, summary_expires_at) if v is not None]
    return {'stale': now >= threshold, 'freshness_stale_at': canonical_rfc3339_utc(threshold),
            'freshness_expires_at': canonical_rfc3339_utc(policy_expires_at) if policy_expires_at else None,
            'eligible_by_time': all(now < v for v in expiries)}


GRANT_NULLS = ('sample_hash', 'commitment_id', 'schema_digest', 'seller_dataset_version',
               'sampled_leaf_list_digest', 'scan_attestation_digest', 'rights_basis_digest',
               'rights_basis_code', 'public_preview_permission', 'preview_type', 'content_type')


def withdrawal_candidate(prior, *, disclosure_version, request_id, approved_at):
    old = closed(DisclosureBinding, prior)
    b = dict(old, decision='withdraw', sample_decision='none', disclosure_version=disclosure_version,
             request_id=request_id, approved_at=approved_at, supersedes=old['disclosure_version'],
...
def refresh_candidate(prior, *, disclosure_version, request_id, attested_at, cadence_days):
    old = closed(DisclosureBinding, prior)
    if old['sample_decision'] != 'approved':
        raise LifecycleError('refresh_requires_approval')
    b = dict(old, decision='approve', disclosure_version=disclosure_version, request_id=request_id,
             approved_at=attested_at, last_attested_by_seller_at=attested_at, update_cadence_days=cadence_days,
             supersedes=old['disclosure_version'], expected_current_disclosure_id=old['disclosure_version'])
...
def supersession_candidate(prior, replacement):
    ...
    if b['seller_id'] != old['seller_id'] or b['listing_id'] != old['listing_id'] or b['request_id'] == old['request_id'] or b['sample_decision'] != 'approved':
        raise LifecycleError('supersession_mismatch')
    b.update(supersedes=old['disclosure_version'], expected_current_disclosure_id=old['disclosure_version'])
    return LocalCandidate.validate(b)
...
def validate_retirement_receipts(receipts, *, url, origin):
    ...
    for receipt, method in zip(receipts, ('GET', 'OPTIONS')):
        if set(receipt) != {'url', 'method', 'status', 'captured_at', 'headers', 'no_set_cookie'} or receipt['url'] != url or receipt['method'] != method or receipt['no_set_cookie'] is not True or set(receipt['headers']) != set(HEADER_KEYS):
        ...
        for key, value in receipt['headers'].items():
            if value is not None:
                if type(value) is not str:
                    raise LifecycleError('invalid_retirement_receipt')
                headers[key] = value
        result = capture_receipt(url, method, receipt['status'], headers, captured_at=receipt['captured_at'], origin=origin, retired=True)
        if result != receipt:
            raise LifecycleError('invalid_retirement_receipt')
...
class PreviewJournal:
    ...
    def allocate_fixture(self, binding, *, id_factory=lambda: str(uuid4())):
        ...
        facts = {k: v for k, v in b.items() if k not in {'request_id', 'disclosure_version'}}
        digest = hashlib.sha256(canonical_json_bytes(facts)).hexdigest()
        with self._db() as db:
            row = db.execute('SELECT candidate FROM fixture_allocations WHERE facts_digest=?', (digest,)).fetchone()
            if row:
            ...
            db.execute('INSERT INTO fixture_allocations VALUES (?,?)', (digest, candidate.binding_bytes))
            return candidate
    ...
    def start(self, candidate):
        ...
        with self._db() as db:
            old = db.execute('SELECT candidate FROM candidates WHERE seller=? AND listing=? AND request_id=?', key).fetchone()
            if old:
                if bytes(old[0]) != candidate.binding_bytes:
                    raise LifecycleError('request_id_conflict')
                return key
            db.execute('INSERT INTO candidates(seller,listing,request_id,kind,state,candidate) VALUES (?,?,?,?,?,?)', (*key, candidate.kind, 'building', candidate.binding_bytes))
        return key

    def _transition(self, db, key, new):
        row = db.execute('SELECT state,kind,request FROM candidates WHERE seller=? AND listing=? AND request_id=?', key).fetchone()
        ...
        db.execute('UPDATE candidates SET state=? WHERE seller=? AND listing=? AND request_id=?', (new, *key))
    ...
    def freeze(self, key, request):
        ...
        with self._db() as db:
            old = db.execute('SELECT candidate,request,digest FROM candidates WHERE seller=? AND listing=? AND request_id=?', key).fetchone()
            ...
            db.execute('UPDATE candidates SET request=?,digest=? WHERE seller=? AND listing=? AND request_id=?', (raw, digest, *key))
            self._transition(db, key, 'signed_candidate')
        return raw, digest

    def read(self, key):
        with self._db() as db:
            row = db.execute('SELECT state,candidate,request,digest,receipts FROM candidates WHERE seller=? AND listing=? AND request_id=?', key).fetchone()
        if not row:
            raise LifecycleError('candidate_missing')
        return dict(zip(('state','candidate','request','digest','receipts'), row))

    def retire(self, key, *, publication_store, disclosure_version, sample_hash, url, origin, receipt_reader):
        ...
        receipts = validate_retirement_receipts(receipt_reader(), url=url, origin=origin)
        with self._db() as db:
            db.execute('UPDATE candidates SET receipts=? WHERE seller=? AND listing=? AND request_id=?', (canonical_json_bytes(receipts), *key))
