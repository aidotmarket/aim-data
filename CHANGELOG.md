...
- Login page now matches the ai.market website: Continue with Google, Continue with GitHub, then email and password. Provider buttons open ai.market with that provider preselected when supported by the website.
...
### Fixed
- Secure delivery connections now use the handshake and encryption required by ai.market, allowing queued deliveries to proceed.
- S3-hosted datasets now use the delivery response supported by ai.market.
- Delivery messages are acknowledged so queued work can continue. When ai.market asks AIM Data to slow down, it pauses sending and retries an S3 delivery once.
- Local and S3 deliveries are marked complete only after ai.market confirms success. Rejected deliveries, invalid confirmations, disconnects and confirmation timeouts are recorded as failed.
